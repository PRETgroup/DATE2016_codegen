...
if(is_file("Cells/cells.csv")) {
	...
	foreach($data as $cell) {
		...
		if(is_file("Cells/" . $filename . ".c") && is_file("Cells/" . $filename . ".h")) {
			...
			fwrite($makefile, "\t@gcc -c -O" . $opt_level . " -lm -Wall Cells/Generated/" . $safe_name . ".c -o Objects/" . $safe_name . "\n");
			fwrite($makefile, "\n");
		}
		else {
...
if(is_file("Paths/paths.csv")) {
	...
	foreach($data as $path) {
		...
		if(is_file("Paths/Path.c") && is_file("Paths/Path.h")) {
			...
			fwrite($makefile, "\t@gcc -c -O" . $opt_level . " -lm -Wall Paths/Generated/" . $safe_name . ".c -o Objects/" . $safe_name . "\n");
			fwrite($makefile, "\n");
		}
		else {
...
fwrite($runnable, "#include <cilk/cilk.h>\n");
fwrite($runnable, "\n");
...
fwrite($makefile, "\t@gcc -fcilkplus -O" . $opt_level . " -lm -Wall runnable.c ");
foreach($items as $item) {
