...
if(i == 0) {
	fprintf(fo, "Time, SA, RA, OS, Fast, AV, His, RBB, RVA, RV\n");
}
fprintf(fo, "%f, %f, %f, %f, %f, %f, %f, %f, %f, %f\n", i*STEP_SIZE, SinoatrialNode_data.v, RightAtrium_data.v, Ostium_data.v, Fast_data.v, AtrioventricularNode_data.v, BundleOfHis_data.v, RightBundleBranch_data.v, RightVentricularApex_data.v, RightVentricle_data.v);
fflush(fo);

double SA_stim_voltage = SinoatrialNode_data.v;
double RA_pacemaker_input = RightAtrium_data.v;
double RV_pacemaker_input = RightVentricle_data.v;

unsigned long cycle_step = i % (unsigned long) (0.500 / STEP_SIZE);
if(cycle_step >= (0.010 / STEP_SIZE) && cycle_step < (0.025 / STEP_SIZE)) {
	//SA_stim_voltage = 131.1;
}
if(cycle_step >= (0.010 / STEP_SIZE) && cycle_step < (0.025 / STEP_SIZE)) {
	//RA_pacemaker_input = 131.1;
}
if(cycle_step >= (0.010 / STEP_SIZE) && cycle_step < (0.025 / STEP_SIZE)) {
	//RV_pacemaker_input = 131.1;
}

/* Cell Mappings */

// SA Node
SinoatrialNode_data.v_i_0 = SA_stim_voltage;
...
RightAtrium_data.v_i_2 = RA_pacemaker_input;
...
RightVentricle_data.v_i_2 = RV_pacemaker_input;
...
RVS_RVS1_data.cell2_mode = RightVentricularSeptum1_data.state;
		 ...
		 cilk_spawn SinoatrialNodeRun(&SinoatrialNode_data);

		 cilk_spawn BachmannBundleRun(&BachmannBundle_data);

		 cilk_spawn LeftAtriumRun(&LeftAtrium_data);

		 cilk_spawn LeftAtrium1Run(&LeftAtrium1_data);

		 cilk_spawn RightAtriumRun(&RightAtrium_data);

		 cilk_spawn RightAtrium1Run(&RightAtrium1_data);

		 cilk_spawn CoronarySinusRun(&CoronarySinus_data);

		 cilk_spawn CristaTerminalisRun(&CristaTerminalis_data);

		 cilk_spawn CristaTerminalis1Run(&CristaTerminalis1_data);

		 cilk_spawn OstiumRun(&Ostium_data);

		 cilk_spawn FastRun(&Fast_data);

		 cilk_spawn Fast1Run(&Fast1_data);

		 cilk_spawn SlowRun(&Slow_data);

		 cilk_spawn Slow1Run(&Slow1_data);

		 cilk_spawn AtrioventricularNodeRun(&AtrioventricularNode_data);

		 cilk_spawn BundleOfHisRun(&BundleOfHis_data);

		 cilk_spawn BundleOfHis1Run(&BundleOfHis1_data);

		 cilk_spawn BundleOfHis2Run(&BundleOfHis2_data);

		 cilk_spawn LeftBundleBranchRun(&LeftBundleBranch_data);

		 cilk_spawn LeftBundleBranch1Run(&LeftBundleBranch1_data);

		 cilk_spawn LeftVentricularApexRun(&LeftVentricularApex_data);

		 cilk_spawn LeftVentricleRun(&LeftVentricle_data);

		 cilk_spawn LeftVentricle1Run(&LeftVentricle1_data);

		 cilk_spawn LeftVentricularSeptumRun(&LeftVentricularSeptum_data);

		 cilk_spawn LeftVentricularSeptum1Run(&LeftVentricularSeptum1_data);

		 cilk_spawn CSLeftVentricularRun(&CSLeftVentricular_data);

		 cilk_spawn RightBundleBranchRun(&RightBundleBranch_data);

		 cilk_spawn RightBundleBranch1Run(&RightBundleBranch1_data);

		 cilk_spawn RightVentricularApexRun(&RightVentricularApex_data);

		 cilk_spawn RightVentricleRun(&RightVentricle_data);

		 cilk_spawn RightVentricle1Run(&RightVentricle1_data);

		 cilk_spawn RightVentricularSeptumRun(&RightVentricularSeptum_data);

		 cilk_spawn RightVentricularSeptum1Run(&RightVentricularSeptum1_data);

		 cilk_spawn SA_BBRun(&SA_BB_data);

		 cilk_spawn SA_OSRun(&SA_OS_data);

		 cilk_spawn SA_RARun(&SA_RA_data);

		 cilk_spawn SA_CTRun(&SA_CT_data);

		 cilk_spawn BB_LARun(&BB_LA_data);

		 cilk_spawn LA_LA1Run(&LA_LA1_data);

		 cilk_spawn RA_RA1Run(&RA_RA1_data);

		 cilk_spawn RA1_CSRun(&RA1_CS_data);

		 cilk_spawn CT_CT1Run(&CT_CT1_data);

		 cilk_spawn OS_FastRun(&OS_Fast_data);

		 cilk_spawn Fast_Fast1Run(&Fast_Fast1_data);

		 cilk_spawn OS_SlowRun(&OS_Slow_data);

		 cilk_spawn Slow_Slow1Run(&Slow_Slow1_data);

		 cilk_spawn Fast1_AVRun(&Fast1_AV_data);

		 cilk_spawn Slow1_AVRun(&Slow1_AV_data);

		 cilk_spawn AV_HisRun(&AV_His_data);

		 cilk_spawn His_His1Run(&His_His1_data);

		 cilk_spawn His1_His2Run(&His1_His2_data);

		 cilk_spawn His2_LBBRun(&His2_LBB_data);

		 cilk_spawn LBB_LBB1Run(&LBB_LBB1_data);

		 cilk_spawn LBB1_LVARun(&LBB1_LVA_data);

		 cilk_spawn His2_RBBRun(&His2_RBB_data);

		 cilk_spawn RBB_RBB1Run(&RBB_RBB1_data);

		 cilk_spawn RBB1_RVARun(&RBB1_RVA_data);

		 cilk_spawn LVA_RVARun(&LVA_RVA_data);

		 cilk_spawn LVA_LVRun(&LVA_LV_data);

		 cilk_spawn LV_LV1Run(&LV_LV1_data);

		 cilk_spawn LVA_LVSRun(&LVA_LVS_data);

		 cilk_spawn LVS_LVS1Run(&LVS_LVS1_data);

		 cilk_spawn LVS1_CSLVRun(&LVS1_CSLV_data);

		 cilk_spawn RVA_RVRun(&RVA_RV_data);

		 cilk_spawn RV_RV1Run(&RV_RV1_data);

		 cilk_spawn RVA_RVSRun(&RVA_RVS_data);

		 cilk_spawn RVS_RVS1Run(&RVS_RVS1_data);
