...
void Cell3d_5inputInit(Cell3d_5input* me) {
	...
	me->pacing_rate = 1 / ((autorhythmic_rate / 60.0) * STEP_SIZE);
	#else
	me->pacing_rate = 0;
	#endif
...
void Cell3d_5inputRun(Cell3d_5input* me) {
    ...
    me->g += (gamma_2 * sigma_2 * (me->v_i_2 - me->v)) / (A_m * C_m * distance_2);
    me->g += (gamma_3 * sigma_3 * (me->v_i_3 - me->v)) / (A_m * C_m * distance_3);
    me->g += (gamma_4 * sigma_4 * (me->v_i_4 - me->v)) / (A_m * C_m * distance_4);
