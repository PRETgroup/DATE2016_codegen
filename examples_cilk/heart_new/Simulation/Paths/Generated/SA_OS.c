...
void SA_OSInit(SA_OS* me) {
	...
	me->cell1_v_buffer = malloc(sizeof(double) * SA_OS_CELL1_NUM_ITEMS);
	me->cell1_mode_buffer = malloc(sizeof(unsigned char) * SA_OS_CELL1_NUM_ITEMS);
	
	int i=0;
    for(i=0; i<SA_OS_CELL1_NUM_ITEMS; i++) {
        me->cell1_v_buffer[i] = 0;
        me->cell1_mode_buffer[i] = 0;
    }
    me->cell1_buffer_index = 0;
	
	me->cell2_v_buffer = malloc(sizeof(double) * SA_OS_CELL2_NUM_ITEMS);
	me->cell2_mode_buffer = malloc(sizeof(unsigned char) * SA_OS_CELL2_NUM_ITEMS);

    for(i=0; i<SA_OS_CELL2_NUM_ITEMS; i++) {
...
void SA_OSRun(SA_OS* me) {
    ...
    if(me->cell1_buffer_index >= SA_OS_CELL1_NUM_ITEMS)
        me->cell1_buffer_index = 0;
    ...
    if(me->cell2_buffer_index >= SA_OS_CELL2_NUM_ITEMS)
        me->cell2_buffer_index = 0;
