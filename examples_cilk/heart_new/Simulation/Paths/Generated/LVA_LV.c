...
void LVA_LVInit(LVA_LV* me) {
	...
	me->cell1_v_buffer = malloc(sizeof(double) * LVA_LV_CELL1_NUM_ITEMS);
	me->cell1_mode_buffer = malloc(sizeof(unsigned char) * LVA_LV_CELL1_NUM_ITEMS);
	
	int i=0;
    for(i=0; i<LVA_LV_CELL1_NUM_ITEMS; i++) {
        me->cell1_v_buffer[i] = 0;
        me->cell1_mode_buffer[i] = 0;
    }
    me->cell1_buffer_index = 0;
	
	me->cell2_v_buffer = malloc(sizeof(double) * LVA_LV_CELL2_NUM_ITEMS);
	me->cell2_mode_buffer = malloc(sizeof(unsigned char) * LVA_LV_CELL2_NUM_ITEMS);

    for(i=0; i<LVA_LV_CELL2_NUM_ITEMS; i++) {
...
void LVA_LVRun(LVA_LV* me) {
    ...
    if(me->cell1_buffer_index >= LVA_LV_CELL1_NUM_ITEMS)
        me->cell1_buffer_index = 0;
    ...
    if(me->cell2_buffer_index >= LVA_LV_CELL2_NUM_ITEMS)
        me->cell2_buffer_index = 0;
