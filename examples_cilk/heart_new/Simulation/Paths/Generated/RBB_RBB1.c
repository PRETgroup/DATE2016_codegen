...
void RBB_RBB1Init(RBB_RBB1* me) {
	...
	me->cell1_v_buffer = malloc(sizeof(double) * RBB_RBB1_CELL1_NUM_ITEMS);
	me->cell1_mode_buffer = malloc(sizeof(unsigned char) * RBB_RBB1_CELL1_NUM_ITEMS);
	
	int i=0;
    for(i=0; i<RBB_RBB1_CELL1_NUM_ITEMS; i++) {
        me->cell1_v_buffer[i] = 0;
        me->cell1_mode_buffer[i] = 0;
    }
    me->cell1_buffer_index = 0;
	
	me->cell2_v_buffer = malloc(sizeof(double) * RBB_RBB1_CELL2_NUM_ITEMS);
	me->cell2_mode_buffer = malloc(sizeof(unsigned char) * RBB_RBB1_CELL2_NUM_ITEMS);

    for(i=0; i<RBB_RBB1_CELL2_NUM_ITEMS; i++) {
...
void RBB_RBB1Run(RBB_RBB1* me) {
    ...
    if(me->cell1_buffer_index >= RBB_RBB1_CELL1_NUM_ITEMS)
        me->cell1_buffer_index = 0;
    ...
    if(me->cell2_buffer_index >= RBB_RBB1_CELL2_NUM_ITEMS)
        me->cell2_buffer_index = 0;
