...
void RVA_RVSInit(RVA_RVS* me) {
	...
	me->cell1_v_buffer = malloc(sizeof(double) * RVA_RVS_CELL1_NUM_ITEMS);
	me->cell1_mode_buffer = malloc(sizeof(unsigned char) * RVA_RVS_CELL1_NUM_ITEMS);
	
	int i=0;
    for(i=0; i<RVA_RVS_CELL1_NUM_ITEMS; i++) {
        me->cell1_v_buffer[i] = 0;
        me->cell1_mode_buffer[i] = 0;
    }
    me->cell1_buffer_index = 0;
	
	me->cell2_v_buffer = malloc(sizeof(double) * RVA_RVS_CELL2_NUM_ITEMS);
	me->cell2_mode_buffer = malloc(sizeof(unsigned char) * RVA_RVS_CELL2_NUM_ITEMS);

    for(i=0; i<RVA_RVS_CELL2_NUM_ITEMS; i++) {
...
void RVA_RVSRun(RVA_RVS* me) {
    ...
    if(me->cell1_buffer_index >= RVA_RVS_CELL1_NUM_ITEMS)
        me->cell1_buffer_index = 0;
    ...
    if(me->cell2_buffer_index >= RVA_RVS_CELL2_NUM_ITEMS)
        me->cell2_buffer_index = 0;
