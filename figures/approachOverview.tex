...
%-----------------------------------------------------------------------
\node [fileS] (HA1) {HA ($Def.~\ref{def:ha}$)};
...
\node[below of = PAR, node distance=1.3cm, text width=3.7cm, text centered]
	(TNHA){Network of HA\\ {($Def.~\ref{def:nha}$)}};
...
%Network of SHA
\node [fileS,above right  = -0.8cm and 0.8cm of  GSHA] (SHA1) {\ac{SHA} ($Def.~X$)};
...
\node [fileS, right  = 1.8cm  of  HA2] (FSM1) {FSM ($Def.~X$)};
