...
\node[below of = HA1, node distance = 1.3cm](PAR){\Large $\dots$};
\node [fileS,fill=red!10,below of = PAR, node distance = 1.3cm] (HA2) 
...
\node[below of = FSM1, node distance = 1.3cm](PAR){\Large $\dots$};
\node [fileS,,fill=red!20,below of = PAR, text width=4.6em, node distance = 1.3cm] (FSM2) 
...
\draw [line] (HA2) -- (FSM2);
\draw [line] (FSM1) -| (PC.150) node[ pos=0.4, above, text width=2cm]
{Step~2:  Parallel composition};
\draw [line] (FSM2) -| (PC.210);
