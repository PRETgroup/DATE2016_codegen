...
\node[state, 
label={[shift={(0,0)}]$v < V_T \wedge g(\vec{v}) \leq V_T$}, 
...
\node[state, 
label={[shift={(0,0)}]$v < V_T \wedge g(\vec{v}) > 0$}, 
label={[shift={(-1.5,-2.5)}]\large $\mathbf{q_1}$   }]
(Q1) [node distance=6.3cm, right of=Q0] {\small{ $v^\prime_x {=} v_x {+} \delta (C_{4} v_x {+} C_{7} g(\vec{v}))$ \\ $v^\prime_y {=} v_y {+} \delta (C_{5} v_y {+} C_{8} g(\vec{v}))$ \\ $v^\prime_z {=} v_z {+} \delta (C_{6} v_z {+} C_{9} g(\vec{v}))$ \\ $v^\prime=v^\prime_x - v^\prime_y + v^\prime_z$}};
