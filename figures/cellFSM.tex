...
\path[->] (Q1.200) edge[out=200,in=340] node[align=center] {
	\footnotesize $\{g(\vec{v}) \leq 0 \wedge v < V_T\}$, \\
...
\path[->] (Q0.180) edge[out=180,in=90,distance=1.5cm] node[align=center, shift={(2.5,0.5)}] {
	\footnotesize $\{v < V_T \wedge g(\vec{v}) \leq V_T\}$, \\
	\footnotesize $\left\{\begin{matrix}
		v^\prime_x = v_x + \delta C_{1} v_x \\
		v^\prime_y = v_y + \delta C_{2} v_y \\
		v^\prime_z = v_z + \delta C_{3} v_z \\
		v^\prime=v^\prime_x - v^\prime_y + v^\prime_z
	\end{matrix}\right\}$
} (Q0.90);

\path[->] (Q1.90) edge[out=90,in=0,distance=1.5cm] node[align=center, shift={(-2.5,0.5)}] {
	\footnotesize $\{v < V_T \wedge g(\vec{v}) > 0\}$, \\
	\footnotesize $\left\{\begin{matrix}
	v^\prime_x = v_x + \delta (C_{4} v_x {+} C_{7} g(\vec{v})) \\
	v^\prime_y = v_y + \delta (C_{5} v_y {+} C_{8} g(\vec{v})) \\
	v^\prime_z = v_z + \delta (C_{6} v_z {+} C_{9} g(\vec{v})) \\
	v^\prime=v^\prime_x - v^\prime_y + v^\prime_z
	\end{matrix}\right\}$
} (Q1.0);

\path[->] (Q2.0) edge[out=0,in=-90,distance=1.5cm] node[align=center, shift={(-2.5,-0.5)}] {
	\footnotesize $\{v < V_O - 80.1 \sqrt{\theta}\}$, \\
	\footnotesize $\left\{\begin{matrix}
...
\path[->] (Q3.-90) edge[out=-90,in=180,distance=1.5cm] node[align=center, shift={(2.5,-0.5)}] {
	\footnotesize $\{v > V_R\}$, \\
	\footnotesize $\left\{\begin{matrix}
