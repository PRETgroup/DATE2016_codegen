...
void PathInit(Path* me) {
	...
	me->cell1_v_buffer = malloc(sizeof(double) * Path_CELL1_NUM_ITEMS);
	me->cell1_mode_buffer = malloc(sizeof(unsigned char) * Path_CELL1_NUM_ITEMS);
	
	int i=0;
    for(i=0; i<Path_CELL1_NUM_ITEMS; i++) {
        me->cell1_v_buffer[i] = 0;
        me->cell1_mode_buffer[i] = 0;
    }
    me->cell1_buffer_index = 0;
	
	me->cell2_v_buffer = malloc(sizeof(double) * Path_CELL2_NUM_ITEMS);
	me->cell2_mode_buffer = malloc(sizeof(unsigned char) * Path_CELL2_NUM_ITEMS);

    for(i=0; i<Path_CELL2_NUM_ITEMS; i++) {
...
void PathRun(Path* me) {
    ...
    if(me->cell1_buffer_index >= Path_CELL1_NUM_ITEMS)
        me->cell1_buffer_index = 0;
    ...
    if(me->cell2_buffer_index >= Path_CELL2_NUM_ITEMS)
        me->cell2_buffer_index = 0;
    ...
    switch (me->state) {
        ...
        else if(me->k*STEP_SIZE >= cell1_conduction_time) {
        ...
        break;
        case (replay_cell1):
        if(me->k*STEP_SIZE >= cell2_charge_time) {
            k_u = 1;
            state_u = idle;
        }
        else {
        ...
        else if(me->k*STEP_SIZE >= cell2_conduction_time) {
        ...
        break;
        case (replay_cell2):
        if(me->k*STEP_SIZE >= cell1_charge_time) {
            k_u = 1;
            state_u = idle;
        }
        else {
