...
void LVS_LVS1Init(LVS_LVS1* me) {
	...
	me->cell1_v_buffer = malloc(sizeof(double) * LVS_LVS1_CELL1_NUM_ITEMS);
	me->cell1_mode_buffer = malloc(sizeof(unsigned char) * LVS_LVS1_CELL1_NUM_ITEMS);
	
	int i=0;
    for(i=0; i<LVS_LVS1_CELL1_NUM_ITEMS; i++) {
        me->cell1_v_buffer[i] = 0;
        me->cell1_mode_buffer[i] = 0;
    }
    me->cell1_buffer_index = 0;
	
	me->cell2_v_buffer = malloc(sizeof(double) * LVS_LVS1_CELL2_NUM_ITEMS);
	me->cell2_mode_buffer = malloc(sizeof(unsigned char) * LVS_LVS1_CELL2_NUM_ITEMS);

    for(i=0; i<LVS_LVS1_CELL2_NUM_ITEMS; i++) {
...
void LVS_LVS1Run(LVS_LVS1* me) {
    ...
    if(me->cell1_buffer_index >= LVS_LVS1_CELL1_NUM_ITEMS)
        me->cell1_buffer_index = 0;
    ...
    if(me->cell2_buffer_index >= LVS_LVS1_CELL2_NUM_ITEMS)
        me->cell2_buffer_index = 0;
