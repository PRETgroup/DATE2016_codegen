...
void OS_SlowInit(OS_Slow* me) {
	...
	me->cell1_v_buffer = malloc(sizeof(double) * OS_Slow_CELL1_NUM_ITEMS);
	me->cell1_mode_buffer = malloc(sizeof(unsigned char) * OS_Slow_CELL1_NUM_ITEMS);
	
	int i=0;
    for(i=0; i<OS_Slow_CELL1_NUM_ITEMS; i++) {
        me->cell1_v_buffer[i] = 0;
        me->cell1_mode_buffer[i] = 0;
    }
    me->cell1_buffer_index = 0;
	
	me->cell2_v_buffer = malloc(sizeof(double) * OS_Slow_CELL2_NUM_ITEMS);
	me->cell2_mode_buffer = malloc(sizeof(unsigned char) * OS_Slow_CELL2_NUM_ITEMS);

    for(i=0; i<OS_Slow_CELL2_NUM_ITEMS; i++) {
...
void OS_SlowRun(OS_Slow* me) {
    ...
    if(me->cell1_buffer_index >= OS_Slow_CELL1_NUM_ITEMS)
        me->cell1_buffer_index = 0;
    ...
    if(me->cell2_buffer_index >= OS_Slow_CELL2_NUM_ITEMS)
        me->cell2_buffer_index = 0;
