...
void LV_LV1Init(LV_LV1* me) {
	...
	me->cell1_v_buffer = malloc(sizeof(double) * LV_LV1_CELL1_NUM_ITEMS);
	me->cell1_mode_buffer = malloc(sizeof(unsigned char) * LV_LV1_CELL1_NUM_ITEMS);
	
	int i=0;
    for(i=0; i<LV_LV1_CELL1_NUM_ITEMS; i++) {
        me->cell1_v_buffer[i] = 0;
        me->cell1_mode_buffer[i] = 0;
    }
    me->cell1_buffer_index = 0;
	
	me->cell2_v_buffer = malloc(sizeof(double) * LV_LV1_CELL2_NUM_ITEMS);
	me->cell2_mode_buffer = malloc(sizeof(unsigned char) * LV_LV1_CELL2_NUM_ITEMS);

    for(i=0; i<LV_LV1_CELL2_NUM_ITEMS; i++) {
...
void LV_LV1Run(LV_LV1* me) {
    ...
    if(me->cell1_buffer_index >= LV_LV1_CELL1_NUM_ITEMS)
        me->cell1_buffer_index = 0;
    ...
    if(me->cell2_buffer_index >= LV_LV1_CELL2_NUM_ITEMS)
        me->cell2_buffer_index = 0;
